...
if ($_POST['bannerid']) {
    ...
    if ($Resultado['id'] != null){
     ...
     if($Executar == 0){
         echo '{"result":false,"msg":"Você já possui esse Banner."}';
         exit;
     }elseif($Executar == 1){
        ...
        if ($stmt2){
            ...
            $stmt3 = $ConnOK->prepareStatment('UPDATE cases_banners SET users = :weaponsidcase WHERE id = :id');
            $stmt3->execute(array(
                ':id' => $Banner,
                ':weaponsidcase' => $TotalC
            ));
        }
        echo '{"result":true,"msg":"Banner Comprado!"}';
        exit;
     }
